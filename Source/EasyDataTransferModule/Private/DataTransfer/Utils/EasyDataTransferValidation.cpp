...
const struct FLogCategoryBase& FEasyDataTransferValidation::GetLogCategory()
{
	return LogEasyDataTransferValidation;
}

bool FEasyDataTransferValidation::ValidateChunk(const FEasyDataChunk& Chunk, const TCHAR* Context)
{
	const FLogCategoryBase& LogCategory = GetLogCategory();
	
	// Validate transfer handle
	if (!FEasyDataTransferHandleGenerator::IsValidHandle(Chunk.TransferHandle))
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Invalid transfer handle %d"), Context, Chunk.TransferHandle);
		return false;
	}
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Invalid chunk indices - Handle:%d, Index:%d, Total:%d"), 
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Too many chunks %d for handle %d (max %d)"), 
	...
	if (Chunk.Data.Num() < 0)
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Negative data size %d for handle %d"), 
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Data size %d exceeds maximum %d for handle %d"), 
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Checksum mismatch for handle %d - Expected:%d, Calculated:%d"), 
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Empty data with non-zero checksum %d for handle %d"), 
...
bool FEasyDataTransferValidation::ValidateTransferParameters(int32 Handle, int32 TotalChunks, int32 TransferSize, const TCHAR* Context)
{
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Invalid total chunks %d for handle %u (max %d)"), 
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Invalid transfer size %d for handle %u (max %d)"), 
...
bool FEasyDataTransferValidation::ValidateCompressionParameters(const TArray<uint8>& Input, int32 CompressedSize, const TCHAR* Context)
{
	...
	if (Input.Num() <= 0)
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Invalid input size %d for compression"), Context, Input.Num());
		return false;
	}
	...
	if (CompressedSize > 0)
	{
		...
		{
			UE_LOG_REF(LogCategory, Warning, TEXT("%s: Suspicious compression ratio %.2f, rejecting"), Context, CompressionRatio);
			return false;
		}
	}
...
bool FEasyDataTransferValidation::ValidateDecompressionParameters(const TArray<uint8>& CompressedData, int32 UncompressedSize, const TCHAR* Context)
{
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Invalid decompression parameters - Compressed:%d, Uncompressed:%d"), 
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Suspicious expansion ratio %.2f (compressed:%d -> uncompressed:%d), rejecting"), 
	...
	{
		UE_LOG_REF(LogCategory, Warning, TEXT("%s: Uncompressed size %d exceeds maximum %d"), 
